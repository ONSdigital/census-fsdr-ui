...
def acc_generation(content):
  ...
  acc_gen.append(
      """<div class="details__heading"><span class="details__title u-fs-r--b">Expand for ID</span></div></summary>"""
  )
  acc_gen.append(
  ...
  acc_gen.append("""<dt class="metadata__term grid__col col-40@m">""" +
                 str(content) + """</dt>""")
  acc_gen.append(
...
def table_generation(tab_data):
  ...
  for headers in tab_data:
    for header in headers:
      add_headers.append({'value': header, 'aria_sort': 'none'}, )
    break

  for all_data in tab_data:
    for data in all_data:
      tds_data.append({'value': all_data[data]})

    add_data.append({'tds': tds_data})
    tds_data = []
